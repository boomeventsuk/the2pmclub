...
export default async function handler(request: Request, context: Context) {
  ...
  try {
    ...
    const searchable = [event.title, event.slug, event.subtitle, event.description, event.image].filter(Boolean).join(" ");
    ...
    const ogDescription = isEighties
      ...
      : event.subtitle || event.description || "4 hours of iconic 80s, 90s & 00s anthems. All the fun of a proper night out, home by 7-ish.";
